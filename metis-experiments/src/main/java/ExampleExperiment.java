...
public class ExampleExperiment {
     ...
     */
    public static CocoProblem CocoPROBLEM;
    ...
    public static final Function evaluateFunction = new Function() {
        public double[] evaluate(double[] x) {
            return CocoPROBLEM.evaluateFunction(x);
        }

        public double[] evaluateConstraint(double[] x) {
            return CocoPROBLEM.evaluateConstraint(x);
        }
    };
    ...
    public static void exampleExperiment(String suiteName, String observerName, Random randomGenerator) {
        try {
            ...
            while ((CocoPROBLEM = benchmark.getNextProblem()) != null) {

                int dimension = CocoPROBLEM.getDimension();

                /* Run the algorithm at least once */
                for (int run = 1; run <= 1 + INDEPENDENT_RESTARTS; run++) {

                    long evaluationsDone = CocoPROBLEM.getEvaluations() + CocoPROBLEM.getEvaluationsConstraints();
                    long evaluationsRemaining = ((long) dimension * BUDGET_MULTIPLIER) - evaluationsDone;

                    /* Break the loop if the target was hit or there are no more remaining evaluations */
                    if (CocoPROBLEM.isFinalTargetHit() || (evaluationsRemaining <= 0))
                        break;

                    /* Call the optimization algorithm for the remaining number of evaluations */
                    myRandomSearch(evaluateFunction,
                            dimension,
                            CocoPROBLEM.getNumberOfObjectives(),
                            CocoPROBLEM.getNumberOfConstraints(),
                            CocoPROBLEM.getSmallestValuesOfInterest(),
                            CocoPROBLEM.getLargestValuesOfInterest(),
                            CocoPROBLEM.getNumberOfIntegerVariabls(),
                            evaluationsRemaining,
                            randomGenerator);

                    /* Break the loop if the algorithm performed no evaluations or an unexpected thing happened */
                    if (CocoPROBLEM.getEvaluations() == evaluationsDone) {
                        System.out.println("WARNING: Budget has not been exhausted (" + evaluationsDone + "/"
                                + dimension * BUDGET_MULTIPLIER + " evaluations done)!\n");
                        break;
                    } else if (CocoPROBLEM.getEvaluations() < evaluationsDone)
                        System.out.println("ERROR: Something unexpected happened - function evaluations were decreased!");
                }

                /* Keep track of time */
                timing.timeProblem(CocoPROBLEM);
            }
